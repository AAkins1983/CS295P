window.onload = setupPage;		// When page loads it calls function setupPage
...
function setupPage()
{
	...
	{
		if (anchors[i].className == "hasEvents")		// If anchor tag has class of hasEvents
		{
			anchors[i].onmouseover = displayEvents;
			anchors[i].onmouseout = clearEvents;
			anchors[i].onclick = cancelClick;		// Don't want click on anchor to take to another page
		}
	}
}
...
function cancelClick()		// To cancel deafault event of action
{
	return false;
}
